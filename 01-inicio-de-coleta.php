...
<form method="POST">
    ...
    <select id="codigofuncionario" name="codigofuncionario">
        <option>Escolha a Matricula</option>
        <?php
        $sql = $pdo->prepare("SELECT * FROM funcionarios  WHERE funcao = 'MOTORISTA' AND setor = 'DOMICILIAR' ORDER BY matricula");
        $sql->execute();
        $fetchAll = $sql->fetchAll();
    ...
    <input hidden id="matricula" name="matricula_motorista" type="text">
    <label>Nome</label><br>
    <input readonly id="nome" name="nome" type="text"><br>
    <label>KM</label><br>
